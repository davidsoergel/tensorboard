...
import { SlimGraph, Metanode, GroupNode, OpNode } from 'src/store/graph/legacy/graph';
...
export class MetanodeComponent implements OnInit {
  ...
  ngOnInit() {
    this.name$ = this.metanode$.pipe(
      filter(m => m !== null), switchMap((m)=>of(m.name)))
    this.childNames$ = this.metanode$.pipe(filter(m => m !== null), switchMap((m)=>of(m.metagraph.nodes())));
  }
